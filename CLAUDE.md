...
npm run build         # Build tokens then build for production (output: dist/)
npm run build:tokens  # Regenerate src/styles/primitives.css from src/tokens/
npm run preview       # Build tokens then preview production build locally
```
...
shadow-primitives.css — hand-authored shadow tokens (shadows are not built via Style Dictionary)
primitives.css        — auto-generated by Style Dictionary from src/tokens/ — do not edit directly
...
**Two-layer token system:** `primitives.css` is auto-generated from DTCG source tokens in `src/tokens/` — run `npm run build:tokens` to regenerate. It defines named raw values (`--color-clay-400: oklch(0.685 0.13 31)`). `tokens.css` is hand-authored and maps semantic names to those primitives (`--color-accent: var(--color-clay-400)`). Always use semantic tokens in component styles; never reference primitives directly, and never edit `primitives.css` by hand.
...
Primitive tokens are defined in DTCG format (TypeScript) and compiled to `src/styles/primitives.css` by Style Dictionary v5.

**Source files** in `src/tokens/`:

```
color/clay.ts       — clay palette (oklch), used for accent
color/gray.ts       — gray palette (oklch), used for all neutrals
animation/          — duration, easing, transition composites
spacing/spacing.ts  — --space-xs through --space-5xl (clamp() values)
spacing/borders.ts  — --border-radius-s/m/l
typography/         — font weights, line heights, type scale (clamp() values)
```

**Build script** `src/build-tokens.ts` runs Style Dictionary v5 with `usesDtcg: true` and the built-in `css` transform group. Output goes to `src/styles/primitives.css`.

**Shadows** are not built via Style Dictionary — they're hand-authored in `src/styles/shadow-primitives.css` (the SD shadow pipeline converts colors to hex, losing alpha).

To add or update a primitive value: edit the relevant `src/tokens/*.ts` file, then run `npm run build:tokens`. Never edit `primitives.css` directly.
...
--font-size-subtle/base/h1–h6   (reference --text-size-step-N from primitives)
...
Type scale in `primitives.css`: `--text-size-step-000` through `--text-size-step-10` (fluid `clamp()` values).
Line heights in `primitives.css`: `--text-line-height-flat/fine/standard/loose`.
